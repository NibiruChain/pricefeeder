...
func ErisProtocolPriceUpdate(symbols set.Set[types.Symbol], logger zerolog.Logger) (rawPrices map[types.Symbol]float64, err error) {
	...
	if err != nil {
		...
		metrics.PriceSourceCounter.WithLabelValues(SourceNameErisProtocol, "false").Inc()
		return nil, fmt.Errorf("failed to connect to gRPC endpoint %s: %w", grpcReadEndpoint, err)
	}
	defer func() {
		if closeErr := conn.Close(); closeErr != nil {
			logger.Err(closeErr).Str("source", SourceNameErisProtocol).Msg("failed to close gRPC connection")
		}
	}()
	...
	logger.Debug().
		Str("source", SourceNameErisProtocol).
		Float64("exchange_rate", exchangeRate).
		Str("symbols", fmt.Sprint(symbols)).
		Msg("fetched prices")

	metrics.PriceSourceCounter.WithLabelValues(SourceNameErisProtocol, "true").Inc()
