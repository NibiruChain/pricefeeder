...
func TestTickSource(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		expectedSymbols := []types.Symbol{"tBTCUSDT"}
		expectedPrices := map[types.Symbol]float64{"tBTCUSDT": 250_000.56}

		ts := NewTickSource(expectedSymbols, func(symbols []types.Symbol) (map[types.Symbol]float64, error) {
	...
	t.Run("logs on price update errors", func(t *testing.T) {
		...
		ts := NewTickSource([]types.Symbol{"tBTCUSDT"}, func(symbols []types.Symbol) (map[types.Symbol]float64, error) {
