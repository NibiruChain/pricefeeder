...
func newPriceProvider(source types.Source, sourceName string, pairToSymbolsMap map[common.AssetPair]types.Symbol, logger zerolog.Logger) *PriceProvider {
	pp := &PriceProvider{
		...
		lastPrices:          map[types.Symbol]types.Price{},
	}
	go pp.loop()
	return pp
}
...
func (p *PriceProvider) GetPrice(pair common.AssetPair) types.Price {
	symbol, ok := p.pairToSymbolMapping[pair]
...
func symbolsFromPairsToSymbolsMap(m map[common.AssetPair]types.Symbol) types.Symbols {
	symbols := make(types.Symbols, 0, len(m))
	for _, v := range m {
...
func isValid(price types.Price, found bool) bool {
